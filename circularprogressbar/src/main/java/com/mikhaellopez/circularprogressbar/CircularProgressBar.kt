...
class CircularProgressBar(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    fun autoUpdateProgress(value: Float, timeDelay: Long, isHasAnimation: Boolean = false) {
        handlerProgress?.removeCallbacksAndMessages(null)
        handlerProgress = null
        handlerProgress = Handler(Looper.getMainLooper())
        autoUpdate(value, timeDelay, isHasAnimation)
    }

    fun cancelAutoUpdateProgress() {
        handlerProgress?.removeCallbacksAndMessages(null)
        handlerProgress = null
    }

    private fun autoUpdate(value: Float, timeDelay: Long, isHasAnimation: Boolean = false) {
        handlerProgress?.postDelayed({
            progress += value
